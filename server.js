...
app.post('/api/admin/jadwal', requireAdmin, (req, res) => {
  try {
    ...
    if (!kelurahan || !hari) return res.status(400).json({ error: 'Kelurahan dan hari wajib diisi' });
    const info = db.prepare(`
      INSERT INTO jadwal (kelurahan, hari, waktu, zona, keterangan) VALUES (?, ?, ?, ?, ?)
    `).run(kelurahan, hari, waktu || '07:00 - 10:00', zona || '', keterangan || '');
    res.status(201).json({ success: true, id: info.lastInsertRowid });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.put('/api/admin/jadwal/:id', requireAdmin, (req, res) => {
  try {
    const { kelurahan, hari, waktu, zona, keterangan } = req.body;
    const info = db.prepare(`
      UPDATE jadwal SET kelurahan=?, hari=?, waktu=?, zona=?, keterangan=? WHERE id=?
    `).run(kelurahan, hari, waktu || '07:00 - 10:00', zona || '', keterangan || '', req.params.id);
    if (info.changes === 0) return res.status(404).json({ error: 'Data tidak ditemukan' });
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
